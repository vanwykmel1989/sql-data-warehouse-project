...
Script Purpose:
    ...
    - Uses INSERT with SELECT transformations to insert data from bronze to silver.
	...
	BEGIN TRY
		...
		FROM
			(
				SELECT 
					*,
					ROW_NUMBER() OVER (PARTITION BY cst_id ORDER BY cst_create_date DESC) AS last_entry
				FROM 
		...
		SELECT 
			...
			[sls_quantity],
			CASE 
				WHEN [sls_price] IS NULL OR sls_price <= 0 OR sls_price != (sls_sales / NULLIF(sls_quantity,0)) THEN (sls_sales / NULLIF(sls_quantity,0))
				ELSE sls_price
			END AS sls_price
		FROM 
